...
@baseline_registry.register_trainer(name="vlfm_custom")
class VLFMCustomTrainer(PPOTrainer):
    envs: VectorEnv
    ...
    def _eval_checkpoint(
        ...
        Returns:
            None
        """
        from vlfm.eval_multi import HabitatMultiEvaluator
        from config import load_eval_config
        import sys
        sys.argv.extend(['-c', '/home/finn/Projects/MultiObjectNav/MON/config/mon/eval_multi_conf.yaml'])
        ...
        evaluator = HabitatMultiEvaluator(eval_config.EvalConf, self)
        evaluator.evaluate()
        observations = self.envs.reset()
        batch = batch_obs(observations, device=self.device)
        batch = apply_obs_transforms_batch(batch, self.obs_transforms)  # type: ignore
